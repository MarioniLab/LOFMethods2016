...
```{r, eval=!local, echo=!local}
all.counts <- read.table("../arrayexpress/lncRNA_counts.tsv", header=TRUE, row.names=1, sep="\t",
    colClasses=c(list("character", "integer"), as.list(rep("integer", 768))))
lengths <- all.counts[,1]
all.counts <- all.counts[,-1]
dim(all.counts)
...
```{r, eval=!local, echo=!local}
metadata <- read.table("../arrayexpress/E-MTAB-5308.sdrf.txt", 
